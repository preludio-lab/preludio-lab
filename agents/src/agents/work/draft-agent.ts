...
6. **タイトルの構成とニックネーム**:
   ...
   - 楽曲形式(\`genres\`)は、\`taxonomy.yaml\` の定義に基づいて、多角的に付与してください（例: ["piano-concerto", "sonata-form", "rondo"]）。
7. **洗練された紹介文**: \`description\` は「事実＋フック」構造を守り、ユーザーの知的好奇心を刺激する洗練された日本語（です・ます調）で記述してください。情緒的な煽りや専門用語の羅列は避け、多言語オブジェクト \`{"ja": "..."}\` として出力します。
