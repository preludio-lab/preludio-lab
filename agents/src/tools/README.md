...
エージェントが推論したデータをワークフロー（オーケストレータ）側で検証し、ファイルシステム（ Gitリポジトリ管理下等）へJSON形式で直接書き出すためのインフラストラクチャ層のモジュールです。

- **目的**: データのSingle Source of Truth（唯一の情報源）の担保と、Thin Orchestrator 原則（エージェントに直接的な副作用を持たせず、ワークフローがI/Oを管轄する）の遵守。
...
- **利用方法**: `AgentDataWriterTool` は `BaseAgent` のツールリスト(`tools`引数)として渡すのではなく、ワークフロー(`execute`)の最終ステップにおいてインフラ層の処理として呼び出されるべきです。
