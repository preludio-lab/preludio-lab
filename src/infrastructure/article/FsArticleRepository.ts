...
export class FsArticleRepository implements ArticleRepository {
  ...
  async findMany(criteria: ArticleSearchCriteria): Promise<PagedResponse<Article>> {
    ...
    if (criteria.minReadingLevel) {
      articles = articles.filter(
        (a) => (a.metadata.readingLevel || 0) >= criteria.minReadingLevel!,
      );
    }
    if (criteria.maxReadingLevel) {
      articles = articles.filter(
        (a) => (a.metadata.readingLevel || 0) <= criteria.maxReadingLevel!,
      );
    }
      ...
      for (const category of categories) {
        ...
        if (fs.existsSync(dirPath)) {
          ...
          for (const filePath of files) {
            ...
            const article = await this.parseArticleFile(
              filePath,
              lang,
  ...
  private async parseArticleFile(
    filePath: string,
    lang: string,
