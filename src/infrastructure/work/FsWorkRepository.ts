...
import { WorkRepository, WorkSearchCriteria } from '@/domain/work/WorkRepository';
import { Work } from '@/domain/work/Work';
import { WorkMetadataSchema } from '@/domain/work/WorkMetadata';
import { WorkControlSchema } from '@/domain/work/WorkControl';
...
export class FsWorkRepository implements WorkRepository {
    ...
    async findMany(criteria: WorkSearchCriteria): Promise<Work[]> {
        ...
        if (criteria.genre) {
            works = works.filter((w) => w.metadata.genre === criteria.genre);
        }
        if (criteria.era) {
    ...
    private parseWorkFile(filePath: string): Work | null {
        try {
            ...
            logger.error('Failed to parse work file', e instanceof Error ? e : new Error(String(e)), { filePath });
            return null;
        }
    }
}
