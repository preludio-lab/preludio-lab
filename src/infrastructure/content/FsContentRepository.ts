...
export class FsContentRepository implements IContentRepository {
  ...
  async getContentDetailBySlug(
    ...
    try {
      ...
      this.logger.error(`Markdownファイルの読み込みエラー: ${fullPath}`, error as Error, {
        context: 'FsContentRepository',
      });
      return null;
    }
  }
    ...
    const contents = files
      .map((filePath) => {
        ...
        try {
          ...
          this.logger.warn(`メタデータのパースエラー (${filePath}):`, {
            context: 'FsContentRepository',
            error: e,
          });
          return null;
  ...
  async getLatestContentSummariesByCategory(
    lang: string,
    category: string,
    limit: number,
