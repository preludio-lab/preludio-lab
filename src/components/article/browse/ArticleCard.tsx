    ...
    >
      <Link href={`/${lang}/${category}/${slug}`} className="flex flex-col h-full">
        ...
        <div className="p-6 flex-grow flex flex-col">
          ...
          <p className="text-sm text-tertiary mb-2 font-medium italic">{composerName}</p>
          ...
          <div className="mt-auto pt-4 flex flex-wrap gap-2">
            {tags?.slice(0, 3).map((tag) => (
              <span
                key={tag}
                className="text-[10px] text-tertiary/60 font-medium px-2 py-0.5 bg-neutral-50/50 rounded-md border border-neutral-100"
              >
                #{tag}
              </span>
            ))}
