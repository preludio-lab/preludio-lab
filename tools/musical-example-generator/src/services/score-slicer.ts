...
export function sliceMusicXML(xmlContent: string, options: SlicerOptions): string {
  ...
  const jsonObj = parser.parse(xmlContent) as XMLElement[];

  // Find score-partwise
  const score = jsonObj.find((node) => node['score-partwise']);
  if (!score) throw new Error('Invalid MusicXML: score-partwise not found');

  const partwise = (score as XMLElement)['score-partwise'] as XMLElement[];

  // --- Part List Filtering ---
  if (options.partId) {
    const partListNode = partwise.find((node) => node['part-list']);
    if (partListNode && (partListNode as XMLElement)['part-list']) {
      const partListChildren = (partListNode as XMLElement)['part-list'] as XMLElement[];

      const filteredList = partListChildren.filter((child) => {
        if (child['score-part']) {
          const attrs = (child as XMLElement)[':@'] as Record<string, string>;
      ...
      (partwise[listIndex] as XMLElement)['part-list'] = filteredList;
    }
  }
  ...
  const filteredPartwiseContent = partwise.filter((node) => {
    if (node['part']) {
      const partAttrs = (node as XMLElement)[':@'] as Record<string, string>;
      ...
      const partContent = node['part'] as XMLElement[];

      const filteredMeasures = partContent.filter((child) => {
        if (child['measure']) {
          const measureAttr = (child as XMLElement)[':@'] as Record<string, string> | undefined;
          ...
          if (options.staffNumber) {
            const measureContent = child['measure'] as XMLElement[];
            const filteredMeasureContent = measureContent.filter((measureChild) => {
              if (
                measureChild['note'] ||
                measureChild['direction'] ||
                measureChild['forward'] ||
                measureChild['backup']
              ) {
                const key = Object.keys(measureChild).find((k) =>
                  ['note', 'direction', 'forward', 'backup'].includes(k),
                ) as string;
                const content = (measureChild as XMLElement)[key] as XMLElement[];
                const staffNode = content.find((n) => n['staff']);
                if (staffNode) {
                  const staffArr = (staffNode as XMLElement)['staff'] as XMLElement[];
                  const staffVal = staffArr?.[0]?.['#text'];
                  if (typeof staffVal === 'string' && parseInt(staffVal) !== options.staffNumber)
                    return false;
                  if (typeof staffVal === 'number' && staffVal !== options.staffNumber)
                    return false;
                }
              }
              ...
              if (measureChild['attributes']) {
                const attrsContent = measureChild['attributes'] as XMLElement[];
                const filteredAttrs = attrsContent.filter((attrNode) => {
                  if (attrNode['clef']) {
                    const clefAttrs = (attrNode as XMLElement)[':@'] as Record<string, string>;
                  ...
                  if (attrNode['staves']) {
                    const stavesContent = (attrNode as XMLElement)['staves'] as XMLElement[];
                    if (stavesContent && stavesContent[0]) {
                      stavesContent[0]['#text'] = 1;
                    }
                  }
                  return true;
                });
  ...
  (score as XMLElement)['score-partwise'] = filteredPartwiseContent;
