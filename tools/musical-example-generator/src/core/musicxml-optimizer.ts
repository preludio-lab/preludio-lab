...
export class MusicXMLOptimizer {
  ...
  optimize(xmlContent: string, options: { removePartGroups?: boolean; resetPositioning?: boolean; alignDynamics?: boolean } = {}): string {
  ...
  private processMeasures(measures: XMLElement[], partIndex: number, options: { resetPositioning?: boolean; alignDynamics?: boolean }): void {
    let globalTupletCounter = 0;
    ...
    measures.forEach((measureNode) => {
      ...
      // 2. 音符 (Tuplets)
      this.optimizeTuplets(measureContent, globalTupletCounter);
  ...
  private optimizeAttributes(measureContent: XMLElement[]): void {
    ...
    if (timeNode) {
      ...
      const beats = (time.find(n => n['beats']) as XMLElement)?.['beats'] as [{ '#text': number }];
      const beatType = (time.find(n => n['beat-type']) as XMLElement)?.['beat-type'] as [{ '#text': number }];
  ...
  private optimizeTuplets(measureContent: XMLElement[], _counter: number): void {
  ...
  private optimizeDirections(measureContent: XMLElement[], partIndex: number, options: { resetPositioning?: boolean }): void {
