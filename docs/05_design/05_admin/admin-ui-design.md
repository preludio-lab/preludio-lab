# Management UI Design Specifications (v2)

Figma AIによって生成され、合意された管理画面のデザイン仕様。本ドキュメントは、管理UI実装における「正解（Source of Truth）」として機能する。

**Version 2 Updates (2026-02-05):**

- 7ヶ国語対応（日本語、英語、ドイツ語、フランス語、イタリア語、スペイン語、中国語）
- リレーション表示機能（作曲家→関連作品、作品→関連フレーズ）
- 名称変更：「譜例」→「フレーズ」
...
- **言語選択ドロップダウン:** サイドバー内に配置。7言語（日本語、英語、ドイツ語、フランス語、イタリア語、スペイン語、中国語）から選択可能。
- **ナビゲーション項目:** 作曲家、作品、フレーズ（旧：譜例）、記事。

### 5.2. Data Table

- 固定幅のカラム設定。
- 行のホバーエフェクト。
- 「アクション」カラム（編集/削除/詳細）の右端配置。
- **詳細表示アイコン:** 目のアイコンで詳細画面へ遷移。

### 5.3. Detail Screens (新規)

- **作曲家詳細:** 基本情報 + 「関連作品」タブでリレーション表示。
- **作品詳細:** 基本情報 + 「関連フレーズ」タブでリレーション表示。
- **多言語入力:** 各テキストフィールドは選択中の言語で表示・編集可能。

### 5.4. Phrase Editor (フレーズエディタ)

- **メタデータ入力:** タイトル、作品との紐付け、小節範囲など。
- **SVGプレビュー:** 事前生成されたSVGファイルの表示（MusicXMLのリアルタイムレンダリングは不要）。
