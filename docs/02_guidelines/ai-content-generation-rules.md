...
| ティア       | 対象                 | 文字数目安    | 必須要素                                        |
| ------------ | -------------------- | ------------- | ----------------------------------------------- |
| **Flagship** | 有名曲、検索流入の柱 | 3,000文字以上 | 全セクション網羅、フレーズ3つ以上、比較聴き比べ |
| **Standard** | 中堅曲、専門的な曲   | 1,500文字以上 | 基本構成網羅、フレーズ1つ以上                   |
...
- **フレーズ (Phrase)**:
  - **ABC記譜法の直接生成は不要**。
  - 代わりに「どのフレーズを提示すべきか」を特定し、`Phrase`コンポーネント（現在はプレースホルダー）を配置する意図をコメント等で残す。
...
- **運用**:
  ...
  - **Slugの一致**: 記事のディレクトリ/ファイル名の Slug 部分は、マスタデータ (`data/`) の `slug` フィールドおよびファイル名と完全一致させること。
  - **Slug規則**: Kebab-Caseを採用する。番号が含まれる場合は `no-5` のようにハイフンで分離する (例: `symphony-no-5`)。
...
- **titleComponents**: タイトルを構成する各要素を分離・統合して管理するオブジェクト。楽曲（Work）および楽章（WorkPart）のメタデータにおいて必須項目。

| フィールド   | 定義                                        | 役割                                                                                                                               | フォーマット例                                                                                          |
| :----------- | :------------------------------------------ | :--------------------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------ |
| **prefix**   | **体系的識別子**<br>(Systematic Identifier) | 楽曲を体系的に分類・識別するための「ジャンル名 + 番号」。これ単体では楽音の実体を持たない「枠」。                                  | **ja**: 交響曲第5番 / ピアノソナタ第14番 / 第1楽章<br>**en**: Symphony No. 5 / Piano Sonata No. 14 / I. |
| **content**  | **実質的タイトル**<br>(Substantive Title)   | 楽曲の固有性を決定づける情報。作曲家が付与した「固有タイトル」、またはそれが存在しない場合は識別子を補完する「テンポ」や「調性」。 | **ja**: ハ短調 / くるみ割り人形 / 海<br>**en**: in C minor / The Nutcracker / La Mer                    |
| **nickname** | **通称**<br>(Colloquial Name)               | 一般大衆や後世によって付けられた、親しみやすい呼び名。                                                                             | **ja**: 「運命」 / 「月光」<br>**en**: "Fate" / "Moonlight"                                             |

- **表記バリエーションと適用例 (Examples)**:

| パターン              | prefix             | content                       | nickname | 統合タイトル (ja)                     |
| :-------------------- | :----------------- | :---------------------------- | :------- | :------------------------------------ |
| **汎用曲 (番号)**     | 交響曲第5番        | ハ短調                        | 「運命」 | 交響曲第5番 ハ短調 「運命」           |
| **汎用曲 (番号)**     | ピアノソナタ第14番 | 嬰ハ短調                      | 「月光」 | ピアノソナタ第14番 嬰ハ短調 「月光」  |
| **汎用曲 (番号なし)** | (なし)             | 幻想即興曲 嬰ハ短調           | (なし)   | 幻想即興曲 嬰ハ短調                   |
| **固有タイトル**      | (なし)             | くるみ割り人形                | (なし)   | くるみ割り人形                        |
| **固有タイトル**      | (なし)             | 海                            | (なし)   | 海                                    |
| **楽章 (汎用)**       | 第1楽章            | アレグロ・コン・ブリオ ハ短調 | (なし)   | 第1楽章 アレグロ・コン・ブリオ ハ短調 |
| **楽章 (固有)**       | 第4曲              | 木星、快楽をもたらす者        | (なし)   | 第4曲 木星、快楽をもたらす者          |
| **楽章 (固有)**       | 第1幕              | (なし)                        | (なし)   | 第1幕                                 |

- **重複の排除**:
  - `popularTitle` は `titleComponents.nickname` と統合されたため、マスタデータJSONのトップレベルフィールドとしては**使用しない**。
- **意味的整合性**:
  - 各言語のフィールドは必ず同じ情報の翻訳であること。言語間で「テンポ」と「楽章番号」を混ぜない。

### 5.1 スキーマバージョンとトレーサビリティ (Versioning & Traceability)

大量生産フェーズにおける品質管理のため、マスタデータJSONには以下のシステム管理用フィールドを必ず含めてください。

- **\_schemaVersion**: `1.1.0` (または最新のスキーマバージョン)
- **\_generatorMeta**:
  - `model`: 使用したモデル名 (例: `gemini-3-flash-preview`)
  - `generatedAt`: 生成日時 (ISO 8601)
  - `promptVersion`: 使用したプロンプトの識別子 (Gitハッシュ等)
...
...
<Phrase id="theme-1" description="第1主題" />
...
