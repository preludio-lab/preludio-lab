...
- **多言語データの意味的整合性 (Semantic Integrity)**:
  ...
  - 楽章（WorkPart）において `isNameStandard: true` の場合、`title` には以下の「統合タイトルルール」を適用し、情報の網羅性を確保すること。

**統合タイトルルール (Consolidated Title Rule)**:

- 構成: `{prefix} {content} {nickname}`
- **prefix**:
  - ja: 「第n楽章」「第n番」「第n幕」等
  - en: 「I.」「II.」「No. n」「Act n」等 (ローマ数字や略称を使用)
- **content**:
  - 固有の曲名がある場合はそれを優先。
  - ない場合は「テンポ表示 + 調性」を記述。日本語は「アレグロ・コン・ブリオ ハ短調」、英語は「Allegro con brio in C minor」。
- **nickname**:
